(() => {
  ...
  const state = { running: false, timer: null, wake: null, screenLock: null, licenseTimer: null, rows: new Map(), muted: false, errorTimer: null, audioContext: null, deviceId: '', licensed: false, requestErrors: 0, lastError: '' };
  ...
  async function keepScreenOn() {
    if (!('wakeLock' in navigator) || state.screenLock) return;
    try { state.screenLock = await navigator.wakeLock.request('screen'); } catch {}
  }
  ...
  async function start() {
    ...
    state.running = true; $('start').disabled = true; $('start').textContent = '▶ Checking Stock...'; $('stop').disabled = false; saveSettings(); unlockAudio(); keepScreenOn();
    try {
      while (state.running) {
        try {
          await scanOnce();
      ...
      clearTimeout(state.timer); state.timer = null; state.wake = null; state.running = false; releaseScreenLock(); $('start').disabled = false; $('stop').disabled = true;
    }
  }
